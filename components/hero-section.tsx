...
export function HeroSection() {
  ...
  useEffect(() => {
    const updateMetrics = () => {
      setDashboardMetrics(prev => ({
        ...
        progress: {
          keywords: oscillate(prev.progress.keywords, 60, 70),
          traffic: oscillate(prev.progress.traffic, 75, 85),
          conversions: oscillate(prev.progress.conversions, 40, 50)
        }
      }))
  ...
  const oscillate = (current: number, min: number, max: number) => {
