...
export function BuiltForResultsSection() {
  ...
  useEffect(() => {
    ...
    const updateRedSegments = () => {
      ...
      const timeout = setTimeout(updateRedSegments, 150)
  ...
  useEffect(() => {
    ...
    const counterAnimation = anime({
      ...
      complete: () => {
        // Slower animation from 150 to 400
        const slowAnimation = anime({
          targets: { value: 150 },
          value: 400,
          duration: 60000, // Very slow increase
          easing: 'linear',
          round: 1,
          update: (anim) => {
            setSpeedValue(Math.floor(anim.animations[0].currentValue))
        ...
        animationsRef.current.push(slowAnimation)
      }
    })
    
    animationsRef.current.push(counterAnimation)
  ...
  return (
    <div ref={sectionRef} className="bg-[#1a1a1a] rounded-3xl p-8 shadow-xl h-full">
      {/* Header */}
      <div className="mb-8">
